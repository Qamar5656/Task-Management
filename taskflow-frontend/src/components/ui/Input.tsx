      ...
      <div className="w-full mb-4">
        ...
        <div className="relative group">
          ...
          <input
            ...
            type={inputType}
            className={`
              w-full bg-slate-900/50 border rounded-lg py-2.5 px-4 text-slate-100 placeholder-slate-500
              focus:outline-none focus:ring-2 focus:ring-indigo-500/50 focus:border-indigo-500 transition-all duration-200
              ${Icon ? 'pl-11' : ''} 
              ${isPassword ? 'pr-11' : ''}
              ${error ? 'border-red-500/50 focus:ring-red-500/50 focus:border-red-500' : 'border-slate-700/50 hover:border-slate-600'}
              ${className}
            `}
            {...props}
          />
          {isPassword && (
            <button
              ...
              className="absolute inset-y-0 right-0 pr-3.5 flex items-center text-slate-500 hover:text-indigo-400 transition-colors z-10 focus:outline-none cursor-pointer"
            >
