      ...
      {isLoading ? (
        ...
        <>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
            {[
              { label: 'Total Workspaces', value: workspaces.length.toString(), icon: LayoutTemplate, color: 'text-indigo-600 dark:text-indigo-400', bg: 'bg-indigo-50 dark:bg-indigo-500/10' },
              { label: 'Active Projects', value: '0', icon: Users, color: 'text-blue-600 dark:text-blue-400', bg: 'bg-blue-50 dark:bg-blue-500/10' },
              { label: 'Tasks Completed', value: '0', icon: CheckCircle2, color: 'text-emerald-600 dark:text-emerald-400', bg: 'bg-emerald-50 dark:bg-emerald-500/10' },
              { label: 'Upcoming Deadlines', value: '0', icon: Clock, color: 'text-orange-600 dark:text-orange-400', bg: 'bg-orange-50 dark:bg-orange-500/10' },
            ].map((stat, i) => (
              <motion.div
                key={stat.label}
          ...
          {workspaces.length === 0 ? (
            ...
            <div className="flex-1 bg-white/70 dark:bg-slate-900/50 backdrop-blur-md border border-slate-200 dark:border-slate-800 rounded-3xl p-8">
              <h3 className="text-xl font-bold text-slate-900 dark:text-white mb-4 font-heading">Recent Activity</h3>
              <p className="text-slate-500 dark:text-slate-400 text-sm">You have {workspaces.length} active workspaces. Select one from the sidebar to view projects!</p>
            </div>
          )}
        </>
      )}
